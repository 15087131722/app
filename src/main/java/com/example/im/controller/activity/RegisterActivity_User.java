...
public class RegisterActivity_User extends AppCompatActivity {
    ...
    private void initView() {
        et_login_name=(EditText) findViewById(R.id.enroll_name_user);
        et_login_pwd = (EditText) findViewById(R.id.enroll_pwd_user);
        bt_login_regist = findViewById(R.id.bt_enroll_user);
    }
    ...
    private void regist() {
        ...
        if (TextUtils.isEmpty(registName) || TextUtils.isEmpty(registPwd)) {
            Toast.makeText(RegisterActivity_User.this,"输入的用户名或密码不能为空",Toast.LENGTH_SHORT).show();
            return;
        }
...
//                    }
                    //去环信服务器注册账号
                    EMClient.getInstance().createAccount(registName+"User",registPwd);
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(RegisterActivity_User.this,"注册失败"+e.toString(),Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            }
